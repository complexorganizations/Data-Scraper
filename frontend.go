...
func uiEditSettings() string {
	...
	page +=			`	</div>
				...
				<tr><th>UserAgents</th><td><input id="settings_captcha" type="text" value="`+settings.Captcha+`"></td></tr>
				<tr>
					<th>User agents</th>
					<td>
						<div id="proxies">`
...
func uiSelectElement(index int) string {
	resp, err := http.Get("https://" + smap.StartURL[0])
	if err != nil {
		frontendLog(err)
	}
...
func writeJSON() {
	jsonData := jsonType{Settings: settings, Smap: smap}
	dataJson, err := json.MarshalIndent(jsonData, "", "  ")
	if err != nil {
		frontendLog(err)
	}

	err = ioutil.WriteFile("./settings.json", dataJson, 0644)
	if err != nil {
		frontendLog(err)
	}
}
